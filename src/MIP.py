...
class Mip:
    ...
    def updateMIP(self, session):
        ...
        if self.alpha > 0:
            try:
                self.centrality = nx.current_flow_betweenness_centrality(self.mip, True,
                    weight='weight')
            except:
    ...
    def DegreeOfInterestMIPsFocus(self, user, obj, focus_obj):
        ...
        try:
            if self.similarityMetric == 'simple':
                focus_proximity = self.simpleProximity(focus_obj,
                    obj)  # node centrality (apriori component)
            else:
    ...
    def adamicAdarProximity(self, s, t):  # s and t are the mip node IDs, NOT user/obj ids
        ...
        for node in nx.common_neighbors(self.mip, s, t):
            weights = self.mip[s][node]['weight'] + self.mip[t][node][
            ...
            if weights != 0:  # 0 essentially means no connection
                ...
                proximity = proximity + (weights * (1 / (math.log(self.mip.degree(node,
                    weight='weight')) + 0.00000000000000000000000001)))  # gives more weight to "rare" shared neighbors, adding small number to avoid dividing by zero
    ...
    def changeExtent(self, userId, aoNode):
        ...
        if userId in self.users.keys():
            ...
            if self.mip.has_edge(userNode, aoNode):
                fromRevision = self.mip[userNode][aoNode][
                    'lastKnown']  # get the last time the user knew what the value of the object was
        ...
        if focus_obj in self.objects.keys():
            ...
            for ao in aoList:
                ...
                if ((changeExtentSinceLastKnown != 0) | (
                    ...
                    if (
                            ao != focus_ao):  # consider object only if it has changed at least once since agent last known about it
    ...
    def rankChangesGivenUserFocus(self, user, focus_obj, time,
                                  onlySig=True):  # TODO: check correctness and try at some point
        notificationsList = []
        if focus_obj in self.objects.keys():
            focus_ao = self.objects[focus_obj]
